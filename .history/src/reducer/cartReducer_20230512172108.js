...
function cartReducer(state, action) {
  ...
  switch (action.type) {
    ...
    case DECREASE_QUANTITY:
      return {
        cart: state.cart.map(
          (item) => item._id === action.payload._id && item.count > 1 ? {...item, count: item.count - 1} : item._id === action.payload._id && item.count ===1 ? {cart: state.cart.filter((item) => item._id !== action.payload._id)} : 
        ),
      }
